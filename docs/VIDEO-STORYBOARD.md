...
- **Light / engaging** — cold open, transitions, the phone-multiplying animation. Upbeat music, snappier cuts.
- **Professional / rigorous** — the three evidence slots, the v1→v2 hardware section, and the verdict. Music pulls back, cuts slow, on-screen text gets clean and precise, narration steadies. (Default to this tone; keep the playful touches restrained.)
...
| 2 | 0:40–1:05 | **The question.** Whiteboard. Someone uncaps a marker and writes the question; team ponders, debates silently. | Big handwritten: **"When should ONE cluster split into MANY? → and into how many?"** | *(VO, Felicia):* "Our project looks at a core question in distributed systems: when should a single Kubernetes cluster be split into several smaller ones — and into how many? One cluster can only handle so much before its control plane becomes a bottleneck. But splitting has its own cost. We set out to find where that trade-off tips." | Everyone turns to look at the desk |
| 3 | 1:05–1:25 | **The hardware.** All eyes turn to Felicia's Pixel on the desk. **Zoom IN** on the phone. | (none) | *(VO, Felicia):* "We run these experiments on a cluster of Pixel phones — low-power, low-cost hardware that's a realistic stand-in for edge devices. Picture eighteen of them running Kubernetes together." | **ANIMATION #1:** one phone multiplies into a cluster |
| 4 | 1:25–2:15 | **Three lines of investigation.** Establish the method as three tiers, each with its own look. | Tags as each appears: **KIND — simulate**, **KWOK — scale**, **PIXELS — reality** | *(VO, Afraz then Tahseen):* "Testing every layout on physical hardware isn't practical, so we investigated in three tiers. First, **Kind** — lightweight virtual clusters on a laptop — to compare many layouts quickly. Then **KWOK**, to simulate thousands of nodes and find the control plane's limits. And finally, the physical **Pixel** cluster." | Push into first evidence |
| 5 | 2:15–2:45 | **Evidence A — Kind.** Screen-recording of a topology test running + **over-the-shoulder huddle** of the team watching it. Then **printed-graph reveal**: hold the latency-vs-clusters chart to camera, lower it. | "Tier 1 · Simulation (Kind)" + finding caption | *(VO, Afraz):* "At small scale, a single cluster performs best — splitting only adds overhead. The benefit appears as the system grows larger. **[FILL: state the threshold from final chart.]**" | Printed graph → push in → dissolve to next |
| 6 | 2:45–3:10 | **Evidence B — KWOK.** Fast-scrolling terminal, node count climbing, one control-plane Grafana panel. | "Tier 2 · Scale (KWOK)" + finding caption | *(VO, Tahseen):* "We also looked at the upper limit: how large a single cluster can grow before the control plane becomes the bottleneck. **[FILL: the ceiling — e.g. ~65 nodes / the request-rate limit.]** That's the real trigger for splitting — not size alone, but the point where one control plane can't keep up." | Hard cut on a spike |
| 7 | 3:10–3:55 | **Evidence C — Pixels (hardware limits).** Calm, factual: v1 error logs scrolling, then the stable v2 run on the real phone. | "Tier 3 · Reality (Pixels)" → "v1 → v2" | *(VO, Felicia):* "Physical hardware introduced constraints the simulations didn't. The control-plane database was bottlenecked by the phones' flash-storage I/O, and the networking stack dropped packets during startup. Our first version couldn't sustain the federation, so we re-architected it — moving the data store off the constrained system partition let the control plane stabilize. **[FILL: v2 result, or 'results still coming in'.]**" | Slow dissolve to whiteboard |
| 8 | 3:55–4:25 | **The verdict.** Back at the whiteboard/group. Someone circles the answer. Serious tone, clean text. | Clean: **"Split when the control plane saturates — not before. Then split into the fewest clusters that clear the bottleneck."** | *(VO, Blake):* "So — when should one cluster become many? Only when a single control plane can't keep up. And how many? The fewest that clear the bottleneck — because every split adds overhead. On constrained hardware, that overhead is what decides the answer." | Pull back from desk |
| 9 | 4:25–4:40 | **Outro.** **Zoom OUT** from the phone / animation rig (bookends Scene 3). Team together. Optional light blooper at the very end. | Credits: names · CSE 145/237D · "Junkyard: Cluster of Clusters" | *(synced, group):* "Cluster of Clusters. Thanks for watching." | Fade |
